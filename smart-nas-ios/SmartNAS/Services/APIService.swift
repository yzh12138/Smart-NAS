...
class APIService: ObservableObject {
    ...
    func login(serverURL: String, username: String, password: String) async throws -> LoginResponse {
        ...
        guard result.code == 0, let loginData = result.data else {
    ...
    // MARK: - Photo
    func getPhotoList(token: String, page: Int = 1, size: Int = 20, tag: String? = nil, keyword: String? = nil, mediaType: Int? = nil) async throws -> PageResult<Photo> {
        ...
        guard result.code == 0, let data = result.data else { throw APIError.serverError(result.message, result.code) }
        return data
    }
    ...
    func deletePhoto(token: String, id: Int) async throws {
        ...
        guard result.code == 0 else { throw APIError.serverError(result.message, result.code) }
    }
    ...
    // MARK: - Photo Upload
    func uploadPhotos(token: String, files: [(Data, String)], tags: String? = nil, city: String? = nil) async throws -> [Photo] {
        ...
        guard result.code == 0, let photos = result.data else { throw APIError.serverError(result.message, result.code) }
        return photos
    }
    ...
    func createTag(token: String, name: String, color: String = "#409EFF") async throws -> Tag {
        let result: ApiResult<Tag> = try await request(method: "POST", path: "/api/tag", body: ["name": name, "color": color], token: token)
        guard result.code == 0, let data = result.data else { throw APIError.serverError(result.message, result.code) }
        return data
    }
    ...
    func uploadFile(token: String, fileData: Data, fileName: String) async throws -> FileStorage {
        ...
        guard result.code == 0, let file = result.data else { throw APIError.serverError(result.message, result.code) }
        return file
    }
