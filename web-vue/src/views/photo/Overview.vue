<template>
  <div class="overview-page" :key="lang">
        ...
        >
          <div style="display: flex; align-items: center; gap: 12px">
            ...
            <el-tag v-if="selectedIds.length > 0" type="primary"
              >{{ selectedIds.length }} {{ t("overview.selected") }}</el-tag
            >
          </div>
          <div style="display: flex; gap: 12px; align-items: center">
            ...
            <template v-if="photoTab === 'my'">
              <el-select
                v-model="selectedTag"
                :placeholder="t('overview.filterTag')"
                clearable
                style="width: 180px"
              >
                <el-option-group
                  v-if="recentTags.length > 0"
                  :label="t('overview.recentTags')"
                >
                  <el-option
              ...
              <el-popconfirm
                :title="t('review.scanApprove')"
                @confirm="handleBatchScanAll(row)"
              >
              <template #reference>
              <el-button
                v-if="!selectMode"
                ...
                :loading="scanning"
                >{{
                  scanning ? t("review.scanning") : t("review.batchScanAll")
                }}</el-button
              >
              </template>
              </el-popconfirm>
              ...
              <el-button
                ...
                type="warning"
                size="small"
                @click="handleBatchScanSelected"
                :loading="scanning"
                >{{
                  scanning
                    ? t("review.scanning")
                    : t("review.batchScanSelected")
                }}
            ...
            <el-button
              ...
              >{{ t("overview.cancelSelect") }}</el-button
            >
            <el-button v-else size="small" @click="toggleSelectMode">{{
              t("overview.selectMode")
            }}</el-button>
          </div>
        </div>
      </template>
        ...
        >
          <div v-if="selectMode" class="select-check">
            <el-checkbox
              ...
              @click.stop
              @change="togglePhotoSelect(photo.id)"
            />
          </div>
    ...
    >
      <div
        v-if="currentPhoto"
        style="display: flex; gap: 20px; min-height: 500px"
      >
        <!-- 左侧：图片 -->
        <div
          style="
        ...
        >
          <div class="ratio-controls">
            <el-button-group size="small">
              <el-button
                :type="ratio === 'auto' ? 'primary' : ''"
                @click="ratio = 'auto'"
                >Auto</el-button
              >
              <el-button
                :type="ratio === '16:9' ? 'primary' : ''"
                @click="ratio = '16:9'"
                >16:9</el-button
              >
              <el-button
                :type="ratio === '4:3' ? 'primary' : ''"
                @click="ratio = '4:3'"
                >4:3</el-button
              >
              <el-button
                :type="ratio === '1:1' ? 'primary' : ''"
                @click="ratio = '1:1'"
                >1:1</el-button
              >
            </el-button-group>
          ...
          <div class="image-container" :style="containerStyle">
            <img
              :src="`/api/photo/${currentPhoto.id}/original`"
              class="detail-image"
            />
          </div>
          <div class="detail-info">
            <p>
              <strong>{{ currentPhoto.originalName || "" }}</strong>
            </p>
            <p v-if="currentPhoto.city || currentPhoto.province">
              {{ currentPhoto.province || "" }} {{ currentPhoto.city || "" }}
            </p>
            ...
            <div class="detail-actions">
              ...
              <el-button
                ...
                >{{ t("family.shareToFamily") }}</el-button
              >
              <el-popconfirm
                :title="t('recycle.confirmDelete')"
        ...
        >
          <h4 style="margin: 0 0 12px">
            {{ t("overview.comments") }} ({{ comments.length }})
          </h4>
          <div
            class="comment-list"
            style="flex: 1; overflow-y: auto; margin-bottom: 12px"
          >
            ...
            <el-empty
              ...
              :description="t('overview.noComments')"
              :image-size="60"
            />
          </div>
          <div class="comment-input">
            <el-input
              v-model="newComment"
              :placeholder="t('overview.commentPlaceholder')"
            ...
            <el-button
              ...
              >{{ t("overview.commentSubmit") }}</el-button
            >
          </div>
        </div>
      </div>
    ...
    >
      ...
      <div class="fullscreen-image-wrap" @wheel.prevent="handleWheel">
        <img
          :src="`/api/photo/${fullscreenPhotos[fullscreenIndex]?.id}/original`"
          class="fullscreen-image"
          :style="{ transform: `scale(${fullscreenZoom})` }"
        />
      </div>
      <div class="fullscreen-nav" v-if="fullscreenPhotos.length > 1">
        <el-button circle @click="prevPhoto" :disabled="fullscreenIndex === 0"
    ...
    <el-dialog
      ...
      :title="t('family.shareToFamily')"
      width="400px"
    >
      <el-select
        v-model="selectedFamilyId"
        :placeholder="t('family.selectFamily')"
        style="width: 100%"
      >
        <el-option
...
const containerStyle = computed(() => {
  switch (ratio.value) {
    case "16:9":
      return { width: "60%", aspectRatio: "16/9" };
    case "4:3":
      return { width: "50%", aspectRatio: "4/3" };
    case "1:1":
      return { width: "40%", aspectRatio: "1/1" };
    default:
      return { maxWidth: "100%", maxHeight: "60vh" };
  }
});
...
async function loadData() {
  if (selectedTag.value) {
    recentTagIds.value = [
      selectedTag.value,
      ...recentTagIds.value.filter((id) => id !== selectedTag.value),
    ].slice(0, 5);
...
async function loadPhotoTags(photoId) {
  ...
  console.log(res.code === 200 && res.data?.tags)
  if (res.code === 200 && Array.isArray(res.tags)) photoTags.value = res.tags;
}
...
async function submitComment() {
  ...
  await addPhotoComment(currentPhoto.value.id, {
...
function downloadPhoto() {
  if (currentPhoto.value)
    window.open(`/api/photo/${currentPhoto.value.id}/original`);
}
...
async function confirmShare() {
  ...
  const photoIds = selectMode.value
    ? [...selectedIds.value]
...
function openFullscreen() {
  fullscreenPhotos.value =
    photos.value.length > 0 ? photos.value : [currentPhoto.value];
  fullscreenIndex.value = fullscreenPhotos.value.findIndex(
...
function handleKeydown(e) {
  ...
  if (e.key === "ArrowLeft") {
    ...
    if (
      fullscreenZoom.value <= 1 &&
      fullscreenIndex.value < fullscreenPhotos.value.length - 1
    ) {
      fullscreenIndex.value++;
      fullscreenZoom.value = 1;
    }
...
function nextPhoto() {
  if (fullscreenIndex.value < fullscreenPhotos.value.length - 1) {
    fullscreenIndex.value++;
    fullscreenZoom.value = 1;
  }
}
