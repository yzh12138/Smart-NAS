<template>
  <div class="review-page" :key="lang">
    <el-card>
      ...
      >
        ...
        <el-table-column :label="t('review.aiTags')">
          <template #default="{ row }">
            <el-tag
              v-for="tag in parseTags(row.ai_tag_names)"
              :key="tag"
              size="small"
              effect="plain"
              ...
              >{{ tag }}</el-tag
            >
            <span v-if="!row.ai_tag_names" style="color: #999">{{
              t("review.noTags")
            }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column :label="t('review.location')" width="120">
          <template #default="{ row }">
            <span
              v-if="row.gps_lat && row.gps_lng"
              style="font-size: 12px; color: #999"
              >{{ row.gps_lat?.toFixed(4) }},
              {{ row.gps_lng?.toFixed(4) }}</span
            >
    ...
    <!-- 扫描进度弹窗 -->
    <el-dialog
      v-model="scanProgressVisible"
      :title="t('review.scanProgress')"
      width="500px"
      :close-on-click-modal="false"
      :show-close="!scanRunning"
    >
      <div v-if="scanProgress">
        <el-progress
          :percentage="scanProgressPercent"
          :status="scanProgressPercent === 100 ? 'success' : ''"
          style="margin-bottom: 16px"
        />
        <p>
          {{ t("review.scanning") }} {{ scanProgress.current }} /
          {{ scanProgress.total }}
        </p>
        <p
          v-if="scanProgress.currentPhoto"
          style="color: #999; font-size: 12px"
        >
          {{ scanProgress.currentPhoto }}
        </p>
        <p style="margin-top: 8px">
          <el-tag type="success" size="small"
            >{{ t("review.success") }}: {{ scanProgress.success }}</el-tag
          >
          <el-tag type="danger" size="small" style="margin-left: 8px"
            >{{ t("review.failed") }}: {{ scanProgress.failed }}</el-tag
          >
          <el-tag
            v-if="scanProgress.skipped > 0"
            type="info"
            size="small"
            style="margin-left: 8px"
            >{{ t("review.skipped") }}: {{ scanProgress.skipped }}</el-tag
          >
        </p>
      </div>
      <template #footer>
        <el-button v-if="scanRunning && !scanPaused" @click="pauseScan">{{
          t("review.pause")
        }}</el-button>
        <el-button
          v-if="scanRunning && scanPaused"
          type="primary"
          @click="resumeScan"
          >{{ t("review.resume") }}</el-button
        >
        <el-button v-if="scanRunning" type="danger" @click="cancelScan">{{
          t("review.cancelScan")
        }}</el-button>
        <el-button v-if="!scanRunning" @click="scanProgressVisible = false">{{
          t("common.confirm")
        }}</el-button>
      </template>
    </el-dialog>
    ...
    <el-dialog
      ...
      :title="t('review.reviewDetail')"
      width="600px"
    >
      <div v-if="currentReview" style="text-align: center">
        <img
          :src="`/api/photo/${currentReview.id}/original`"
          style="
            max-width: 100%;
            max-height: 400px;
            object-fit: contain;
        ...
        <div style="margin-top: 16px">
          <p>
            <strong>{{ currentReview.original_name || "" }}</strong>
          </p>
          <p v-if="currentReview.city || currentReview.province">
            {{ currentReview.province || "" }} {{ currentReview.city || "" }}
          </p>
          <p
            v-if="currentReview.gps_lat && currentReview.gps_lng"
            style="font-size: 12px; color: #999"
          >
            {{ t("review.location") }}: {{ currentReview.gps_lat?.toFixed(6) }},
            {{ currentReview.gps_lng?.toFixed(6) }}
          </p>
        </div>
        <div style="margin-top: 16px">
        ...
        <div style="margin-top: 12px">
          <el-input
            v-model="customTag"
            :placeholder="t('review.customTagPlaceholder')"
            style="width: 200px; margin-right: 8px"
          />
          <el-button size="small" @click="addCustomTag">{{
            t("review.addTag")
          }}</el-button>
        </div>
      </div>
      <template #footer>
        ...
        <el-button type="primary" @click="submitReview(true)">{{
          t("common.save")
        }}</el-button>
      </template>
    </el-dialog>
...
const scanProgressPercent = computed(() => {
  ...
  return Math.round(
    (scanProgress.value.current / scanProgress.value.total) * 100,
  );
});
...
async function handleBatchScanAll() {
  ...
  const allPhotos = (res.data.records || []).filter(
    (p) => p.mediaType === "image" && !p.aiAnalyzed,
  );
...
function handleBatchScanSelected() {
  const photos = reviewList.value.filter((r) =>
...
function addCustomTag() {
  if (
    ...
    !selectedTags.value.includes(customTag.value.trim())
  ) {
